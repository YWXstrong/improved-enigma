...
function App() {
  ...
  // 【新增】背景和交互状态
  const [backgroundImage, setBackgroundImage] = useState(''); // 背景图片
  const [introduction, setIntroduction] = useState('欢迎使用团队协作模型！这是一个用于团队成员管理的应用。'); // 可编辑介绍文本
  const [isEditingIntro, setIsEditingIntro] = useState(false); // 编辑状态
...
//主界面渲染（已登录状态）
  return (
    <div className="App">
      <div className="background-container" style={{backgroundImage: backgroundImage ? `url(${backgroundImage})` : 'none'}}>
        <div className="background-overlay"></div>
      </div>
      <div className="content-container">
        <div className="interactive-box title-box">
          <h1>团队协作模型</h1>
        </div>

        <div className="interactive-box message-box">
          <p>{message}</p>
        </div>
        ...
        {/* 【新增】可编辑介绍文本 */}
        <div className="interactive-box intro-box" onClick={() => setIsEditingIntro(true)}>
          {isEditingIntro ? (
            <div>
              <textarea
                value={introduction}
                ...
                placeholder="点击编辑介绍文本..."
              />
              <div className="edit-hint">Ctrl+Enter保存</div>
            </div>
          ) : (
            <p>{introduction}</p>
          )}
        ...
        <div className="interactive-box users-title-box">
          <h2>用户列表</h2>
        </div>
        ...
        <div className="interactive-box links-box">
        ...
        {/* 【新增】背景图片切换控件 */}
        <div className="interactive-box background-controls">
          <h3>背景设置</h3>
          <div className="background-buttons">
            <button onClick={() => changeBackground('')}>无背景</button>
            <button onClick={() => changeBackground('https://picsum.photos/800/300?random=1')}>风景1</button>
            <button onClick={() => changeBackground('https://picsum.photos/800/300?random=2')}>风景2</button>
            <button onClick={() => changeBackground('https://picsum.photos/800/300?random=3')}>抽象1</button>
            <button onClick={() => changeBackground('https://picsum.photos/800/300?random=4')}>抽象2</button>
          </div>
        </div>
      </div>
    </div>
